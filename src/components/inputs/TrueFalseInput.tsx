    ...
    <div className="flex flex-col gap-3">
      <div className="flex gap-3">
        <button
          ...
          disabled={disabled}
          className={`min-h-[44px] flex-1 rounded-xl border-2 font-medium transition-all duration-150 focus-visible:outline-hidden disabled:opacity-60 ${
            value === "true"
      ...
      {showCounterexample && onWorkingsChange && (
        <div className="flex flex-col gap-2">
          ...
          <textarea
            ...
            rows={3}
            className="min-h-[80px] rounded-xl border-2 border-zinc-200 bg-card px-4 py-2 text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-hidden focus:ring-2 focus:ring-primary/20 disabled:opacity-60 dark:border-zinc-600"
          />
        </div>
      )}
      <button
